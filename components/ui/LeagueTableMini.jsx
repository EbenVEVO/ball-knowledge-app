...
export const LeagueTableMini = ({club}) => {
...
const [newStandings, setNewStandings] = useState([]);
  const [direction , setDirection] = useState(null);
  const [selectedColumn, setSelectedColumn] = useState(null);

  useEffect(() => {
    const teamIndex = standings.findIndex(standings => standings.team.id === club?.id)
     if (teamIndex - 6 <= 0) 
    {
      setNewStandings( standings.slice(0, 5))
    }
    else
    {
      setNewStandings(standings.slice(teamIndex - 2, teamIndex + 3))
    }
  }, [standings])
...
<View className='flex flex-row   items-center'>
    ...
    const renderRow = (item, index) =>{
        return(
            <> 
            <View className='flex flex-row items-center  '  key={index} style={{backgroundColor: club?.id === item.team.id ? '#A6DEFF' : ''}}>
                ...
                {club?.id === item.team.id ? 
                                  <Text className={`font-supreme text-md ${club?.id === item.team.id ? '':'text-blue-600'}`}>
 ...
 const handleSort = (column) =>{
            ...
            if (column === 'date') {
                const sorted = _.orderBy(newStandings, [
                    (item) => {
                        const [month, day, year] = item.date.split('/')
                        return new Date(year, month - 1, day)
                    }
                ], [newDirection])
                setNewStandings(sorted)
            } 
                ...
                const sorted = _.orderBy(newStandings, [column], [newDirection])
                setNewStandings(sorted)
            }
  ...
  return (
    <View className= ' p-5 w-full '>
      <Text className='text-2xl font-supremeBold' style={{color: colors[0]}}>League Table</Text> 
       <ScrollView horizontal style={{padding:15}} >
                   <FlatList style={styles.tableContainer} className='bg-white rounded-xl ' scrollEnabled={false} data={newStandings} renderItem={({item, index}) => renderRow(item, index)} ListHeaderComponent={renderHeader} nestedScrollEnabled />
               </ScrollView>
    </View>
  )
}
